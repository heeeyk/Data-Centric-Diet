...
from trainer.training_vnet_VOG import trainbox
...
os.makedirs(f'{snapshot_exp}/vog_files', exist_ok=True)
...
if __name__ == "__main__":
    ...
    for i in all_cases:
        if args.dataset == "msd_nih" and 'word' in i:
            ...
            if 'msd' in i or 'nih' in i:
                eval_list.append(i.split('.')[0])
        else:
            train_list.append(i.split('.')[0])

    # ------------------------------------------- ATTENTION ------------------------------------------ #
    dices_root = f'{args.server_path}/HYK/DMDS_save/{args.dataset}/exp4-GraNd/No_Prunning/dice_files'
