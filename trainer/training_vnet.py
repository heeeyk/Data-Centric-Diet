    ...
    for epoch in tqdm.tqdm(range(1, args.max_epoch)):
        ...
        for i_batch, sampled_batch in enumerate(dataloader):
            if sampled_batch:
                ...
                # volume_batch.shape:(batch_size, patch_nums, 1, patch_size, ...)
                ...
                # output.shape:(patch_nums*batch_size, class, patch_size, ...)
                # label.shape:(patch_nums*batch_size, 1, patch_size, ...)
            ...
            if iter_num >= 10000 and iter_num % 1000 == 0:
                ...
                eval_d = []
                for i in range(eval_cases, 2*eval_cases):
                    _, d = test.test0(eval_images[i], eval_labels[i])
                    eval_d.append(d)
                print('\ttrain dsc, mean: {:.4f}, max: {:.4f}, min: {:.4f}'.format(np.mean(eval_d), np.max(eval_d), np.min(eval_d)))
                net.train()
        ...
        if epoch % 10 == 0:
            ...
            #         d = DataMap(args, save_path=args.dm_path, dice_file=dice_file)
            #         del_list = d.data_select(args.del_rate, args.del_mode)
            #         np.save(snapshot_exp, del_list)
            #         d.list_nums(del_list=del_list)
            #
            #         # reload dataset.
