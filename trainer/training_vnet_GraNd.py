...
from trainer.utils import dice, dice_torch, DataMap, recall, load_data_test, false_rate
import time
...
def trainbox(net,
    ...
    for epoch in tqdm.tqdm(range(args.load_epoch, args.max_epoch)):
        ...
        for i_batch, sampled_batch in enumerate(dataloader):
            if sampled_batch:
                ...
                if dice_file is not None:
                    ...
                    batch_grad = torch.zeros(args.batch_size)
                    batch_false = torch.zeros(args.batch_size)
                    ...
                    for i, case_name in enumerate(case_batch):
                        ...
                        batch_false[i] = false_rate(pred[i * args.patch_nums:(i + 1) * args.patch_nums], label_batch[i * args.patch_nums:(i + 1) * args.patch_nums])
                        ...
                        #
                        # grad = volume_batch.grad[i * args.patch_nums:(i + 1) * args.patch_nums].squeeze(1)
                        # grad[label_batch[i * args.patch_nums:(i + 1) * args.patch_nums] != 1] = 0
                        # batch_grad[i] = torch.norm(grad, 2).detach().cpu()
                        ...
                        # dice_file[case_name]['false_rate'] = np.append(dice_file[case_name]['false_rate'],
                        #                                          np.asarray(batch_false[i], dtype='float32'))
                        # dice_file[case_name]['grad'] = np.append(dice_file[case_name]['grad'], np.asarray(batch_grad[i], dtype='float32'))
                        ...
                        total_dice_file[case_name]['dice'] = np.append(total_dice_file[case_name]['dice'], np.asarray(batch_dice[i], dtype='float32'))
                        total_dice_file[case_name]['corr'] = np.append(total_dice_file[case_name]['corr'], np.asarray(batch_corr[i], dtype='float32'))
                        # total_dice_file[case_name]['false_rate'] = np.append(total_dice_file[case_name]['false_rate'],
                        #                                          np.asarray(batch_false[i], dtype='float32'))
                        total_dice_file[case_name]['nums'] += 1
        ...
        if epoch % 10 == 0:
            ...
            np.save(snapshot_exp + f'/dice_files/total_dice.npy', total_dice_file)
            dice_file = {}
            for i, name in enumerate(list(dataset.keys())):
                dice_file.update({
                    f'{name}': {  # 每个样本的名字，我们根据这个来挑选数据
